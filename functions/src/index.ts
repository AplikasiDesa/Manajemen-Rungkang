...
function doPost(e) {
  try {
    ...
    // --- Aksi baru: Unggah file arsip dengan nama khusus ---
    if (action === 'uploadArchiveFile') {
       const { folderId, fileData, fileName } = body;
       ...
       return createJsonResponse({ success: true, fileUrl: file.getUrl(), fileId: file.getId() });
    }
